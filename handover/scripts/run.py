...
def run_model(packaged_data, result_html_path, model_optimality_gap = 0.01, model_time_limit = 60):
    ...
    for p in range(num_slices):
        ...
        # try:
        model, revenue, schedule_dict, revenue_dict = optimize_revenue(initial_soc, packaged_data, periodic_price, periodic_load, final_soc_target=0, cap_settings = [], model_optimality_gap = model_optimality_gap, model_time_limit = model_time_limit)
    ...
    if result_html_path:
        generate_html(result_html_path, bess, service, price_df, result_df, revenue_data, basis['wacc'], service['load'])
